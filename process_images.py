...
def generate_and_upload_image(title, api_key_manager):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Create a highly creative and visually striking prompt
            prompt = f"""Create a stunning, ultra-creative blog header image for: "{title}". 
            
            Style requirements:
            - realistic image
            - Rich, vibrant colors with beautiful gradients
            - Dynamic composition with interesting angles and perspectives
            - Professional photography quality with shallow depth of field
            - Modern, contemporary aesthetic with artistic flair
            - Include subtle bokeh effects and atmospheric elements
            - Use creative visual metaphors and symbolic elements related to the topic
            - Incorporate elegant typography space considerations
            - High contrast and visual impact that stops scrolling
            - Award-winning commercial photography style
            
            Make it visually captivating, premium quality, and emotionally engaging. The image should feel alive and dynamic, not static or boring. Think magazine cover quality with artistic creativity."""
        ...
        try:
            ...
            if new_image_url:
                ...
                if update_markdown_file(file_path, new_image_url):
                    ...
                    # Delete the file from articles-folder
                    if delete_file(file_path):
                        deleted_files.append(file_path.name)
                        print(f"Deleted file from articles-folder: {file_path.name}")
                    else:
                        print(f"Failed to delete file: {file_path.name}")
                else:
